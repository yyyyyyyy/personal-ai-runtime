...
- **场景模版 (Scene Templates)**：4 个预置自动化场景（管理邮件、追踪目标、自动化日报、建立记忆），一键创建工作流
- **集成中心 (Integrations Hub)**：统一展示所有连接器状态与工具列表，支持邮箱、日历、Brave、GitHub、Notion 等
- **加密导出/导入 (Encrypted Sync 基础)**：AES-GCM 加密数据导出，跨设备安全传输 Event Log
