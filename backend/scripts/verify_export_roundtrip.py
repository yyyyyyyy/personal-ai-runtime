...
def main() -> int:
    ...
    with (
        ephemeral_db_path("verify_export_roundtrip.db", prepare=False) as source_path,
        ephemeral_db_path("verify_export_roundtrip_import.db", prepare=False) as import_path,
    ):
        ...
        before = source_kernel.table_counts(
            ("event_log", "conversations", "messages", "memories", "notifications")
        )
        ...
        after = import_kernel.table_counts(
            ("event_log", "conversations", "messages", "memories", "notifications")
        )
        ...
        for key in ("event_log", "conversations", "messages", "memories", "notifications"):
            if before.get(key) != after.get(key):
                print(
                    f"FAIL: count mismatch for {key!r}: "
                    f"before={before.get(key)} after={after.get(key)}",
                    file=sys.stderr,
                )
                failed = True
