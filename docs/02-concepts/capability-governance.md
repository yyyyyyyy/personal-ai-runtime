...
LLM 调用前的审计边界由 [`backend/app/core/runtime/egress/egress_gate.py`](../../backend/app/core/runtime/egress/egress_gate.py) 实现。`prepare_llm_egress(messages, purpose, actor)` 返回 `(messages, audit_meta)`——**messages 原样通过，不做脱敏**——同时发出 `EgressApproved` 事件，分类为 `identity_surface` / `memory_context` / `trajectory_context` / `general`。Brain 与 BrainCompletionMixin 在每次 LLM 调用前调用。
