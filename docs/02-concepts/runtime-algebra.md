...
| 指标 | 当前值 | 目标值 (1 年后) |
|---|---|---|
| `core/runtime/` 文件数 | 61 | ≤ 45 |
| `constants.py` 事件类型数 | 69 (+4 WorkItem) | ≤ 60 |
| `query_state` selector 分支数 | 15 (+1 work_items) | ≤ 10 |
| Fragment 注册数 | 13 | ≤ 10 |
| Governed 投影表数 | 15 (+1 work_items) | ≤ 13 |
| Projector 文件数 | 10 | ≤ 8 |
| 游离单例（不在 container 内） | 4 | 0 |

> v0.5.0: +1 work_items 表/事件/selector 替代了 tasks + actions 两个聚合。净概念数下降。
...
| v0.6 | 替换 TriggerEngine → Reaction | runtime 文件数 61 → 50 |
| v1.0 | 合并 Goal 到 Work 原语 | governed 表 15 → 12 |
