...
| `governance/capability_context.py` | 285 | Capability（快照） | 暂保留（dormant） | v0.4.0 前若无调用方则删 |
...
当前 [`check_concept_growth.py`](../../backend/scripts/check_concept_growth.py) 八项指标全部顶满基线（45/45、50/50…）。开发期（未上生产）的功能推进需遵守以下闸门，避免在日用证据尚浅时继续堆概念：
