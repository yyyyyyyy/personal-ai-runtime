...
| `governance/execution_context` | — | Context | **已删除** (v0.3.0) | ✓ dead code，零生产引用（身份移至 `execution.py`） |
| `governance/capability_context` | — | Capability（快照） | **已删除** | ✓ dormant 代码，零生产引用，清理兑现 |
...
| Legacy Event Adapter | `legacy_event_adapter.py` | **已删除** → 折叠进 `read_ports/` |
| Mixin Protocol | `kernel/_mixin_protocol` | **已删除** (v0.4.0) |
| Execution Shadow Compare | `execution_shadow_compare` | 保留（验证工具，后续移到 tests/） |
...
### 4.6 概念总数基线（2026-07-14）

| 指标 | 当前值 | 目标值 (1 年后) |
|---|---|---|
| `core/runtime/` 文件数 | 57 | ≤ 44 |
| `constants.py` 事件类型数 | 50 | ≤ 50 |
| `query_state` selector 分支数 | 10 | ≤ 10 |
| Fragment 注册数 | 10 | ≤ 10 |
| Governed 投影表数 | 14 | ≤ 11 |
| Projector 文件数 | 6 | ≤ 7 |
| 游离单例（不在 container 内） | 0 (v0.5.1 收编 db/vector_store/runtime_loop 等) | 0 |
| Dead code 文件数 | 0 | 0 |
| God Object 最大 LOC（Kernel/Brain/MCPHub） | 559 | ≤ 1500 |

**抬红线说明（2026-07-14）**：`runtime_files` 44 → 57。原因：`read_ports.py`（单文件）拆成 `read_ports/` 包（14 个域文件），净 +13。**未新增五原语概念**——仍是同一个 Read Port；拆分是为打破 762 LOC 单体、配合 api/ 去 store 直连。一年目标仍 ≤ 44：若 dogfood 判定拆分「不重要」，应折叠回更薄的包（或单文件）把文件数压回去。
...
> **版本约定**：本节及全代码库 docstring 中的 `v0.3.0`…`v1.1` 是**子系统演进阶段标记（phase）**，记录某次概念压缩/迁移发生的节点，不是产品发布版本号。产品版本以 [`backend/app/version.py`](../../backend/app/version.py) 为唯一权威源（当前 `0.2.0`）。phase 编号可超前于产品版本——例如 `v1.1` phase 已完成不代表产品已发布 1.x。
...
| v1.0 ✅ | 合并 Goal 到 Work 原语；删除 goals/grant_events/triggers 等死表 | governed 表 14，app 表 5 |
| v1.1 ✅ | runtime 文件压缩 + 治理折叠 + Phase 4 退休 legacy API | runtime 51→45, god_object 577→559, event_types 56→50 |
| v1.2 ✅ | `read_ports.py` → `read_ports/` 域包（api 去 store） | runtime 44→57（同概念拆包；一年目标仍 ≤44） |

### 5.4 开发期功能闸门（当前阶段）

当前 [`check_concept_growth.py`](../../backend/scripts/check_concept_growth.py) 多数指标顶满基线（57/57、50/50…；`runtime_files` 一年目标仍 ≤44）。开发期（未上生产）的功能推进需遵守以下闸门，避免在日用证据尚浅时继续堆概念：

**优先级倒置原则**：开发期应优先用 dogfood 证据驱动功能（见 [05-engineering/development.md](../05-engineering/development.md)「开发期自用检查」），而非继续压缩概念。「再压缩一轮 runtime」只有在 dogfood 证明某概念阻碍日用时才做。

**新需求的三道闸**：

1. **吞并测试先行**：新需求先尝试用五原语（Event/State/Capability/Work/Context）组合表达。能用现有事件类型 + 投影表 + fragment + capability 表达 → 写为声明/实例，不加 `.py` 文件、不加 `EVENT_*` 常量。
2. **零和强制**：若必须新增概念（新事件类型 / runtime 文件 / fragment），同 PR 必须删除一个等价旧概念，保持 [`check_concept_growth.py`](../../backend/scripts/check_concept_growth.py) 净增长 ≤ 0。
3. **抬红线需说明**：若确实需要抬基线（如 governed 表从 14 → 15 换掉两个旧表），PR 描述必须写明：替换掉的旧概念 + 新基线值，并跑 `make architecture-record` 更新 [`check_concept_growth.py`](../../backend/scripts/check_concept_growth.py) 的 baseline。**禁止静默抬红线。**

**反模式（开发期应避免）**：

- 为了「架构更干净」而压缩概念，但该概念当前并无日用阻碍。
- 新增事件类型而不删旧类型，让 CI 红线静默上涨。
- 在 inbox=0、messages 极少的情况下讨论「MCP 生态标准」或「多 principal」——这些是生产期/扩张期议题。
