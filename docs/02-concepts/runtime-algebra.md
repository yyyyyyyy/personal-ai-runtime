...
**统一要求**：后台异步任务应表达为 WORK（领域 subtype），而不是平行的第二套任务表。若代码中仍存在独立的 `background_tasks`（APP_STORAGE），那是待收敛的实现缺口，不是第二套原语。
...
| BackgroundTask | `Work` 的领域 subtype | 应收敛到统一 Work 模型 |
...
| `query_state` selector 分支数 | 10 |
| Fragment 注册数 | 10 |
| Governed 投影表数 | 14 |
| Projector 文件数 | 6 |
| God Object 最大 LOC（Kernel/Brain/MCPHub） | 559 |
