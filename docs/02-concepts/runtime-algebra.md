...
| 现有概念 | 表达为 | 新概念？ |
|---|---|---|
| Goal | `Work(type=goal) + State(goals)` | **否** |
| Task | `Work(type=task) + dependencies_json` | **否** |
| Action | `Work(type=action, parent_id=goal_id)` | **否** |
| Execution | `Work 的一个 lifecycle stage` | **否** |
...
| Trigger | `subscribe(Event) + invoke(Capability) + produce(Work)` | **否** |
...
| `trigger_engine.py` | 178 | Event + Capability + Work | **否** | **替换为声明式 Reaction** |
...
| AgentInstance | `agent_instance.py` | **已瘦身** → 仅 `SINGLETON_INSTANCE_ID` 常量 |
...
| 指标 | 当前值 | 目标值 (1 年后) |
|---|---|---|
| `core/runtime/` 文件数 | 61 | ≤ 40 |
| `constants.py` 事件类型数 | 64 | ≤ 50 |
| `query_state` selector 分支数 | 14 | ≤ 8 |
| Fragment 注册数 | 13 | ≤ 10 |
| Governed 投影表数 | 14 | ≤ 12 |
| Projector 文件数 | 10 | ≤ 7 |
| 游离单例（不在 container 内） | 4 | 0 |
...
以下指标应配置在 CI 中，任何 PR 突破红线应**阻断合并**：

| 检查项 | 红线 | 测量方法 |
|---|---|---|
| `constants.py` 事件类型数 | ≥ 当前值 + 1 且无删除 | `grep -c 'EVENT_'` |
| `core/runtime/` 文件数 | ≥ 当前值 + 1 且无删除 | `find -name '*.py' | wc -l` |
| `query_state` 新增 selector | +1 且无等价 selector 删除 | diff `if selector ==` 分支 |
| 新增 Fragment 注册 | +1 且无旧 Fragment 删除 | diff `register.py` 的 `_ALL_FRAGMENT_CLASSES` |
| 新增投影表 | +1 且无旧投影表删除 | diff `GOVERNED_TABLES` |

**违反任何一条红线的 PR，必须同时包含删除等同概念的操作。**
...
| v0.5 | 替换 query_state → Repository 端口 | selector 14 → 8 |
| v0.6 | 合并 Goal/Action/Task/Execution → Work | governed 表 14 → 9 |
| v1.0 | 替换 TriggerEngine → Reaction | runtime 文件数 61 → 45 |
