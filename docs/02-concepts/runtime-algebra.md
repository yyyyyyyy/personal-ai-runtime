...
| Task | `WorkItemCreated(work_type=task) → work_items` | 统一到 WorkItem |
| Action | `WorkItemCreated(work_type=action) → work_items` | 统一到 WorkItem |
| Execution | `Work 的一个 lifecycle stage` | 执行记录 |
| BackgroundTask | `Work(type=background)` | — |
...
| Notification | `Event(NotificationCreated)` + push `Context` 通道 | — |
...
> 概念清单是当前运行时的事实快照（见 §4）。一年测试用于评估未来新增概念：只有「无论项目怎么演进，它一定在」的概念才允许加入。
...
`runtime_files` 基线为 57：`read_ports/` 按域拆分为多个文件（同一个 Read Port 概念），一年目标仍 ≤ 44。若 dogfood 判定拆分「不重要」，应折叠回更薄的包。
...
以下指标由 [`backend/scripts/check_concept_growth.py`](../../backend/scripts/check_concept_growth.py) 测量并在 CI 中强制（红线 = 当前基线值，详见 §4.4）。任何 PR 让指标**净增长** > 0 都会被阻断：
...
新增需求需遵守以下闸门，避免在日用证据尚浅时继续堆概念：

**优先级倒置原则**：应优先用 dogfood 证据驱动功能（见 [05-engineering/development.md](../05-engineering/development.md)），而非继续压缩概念。「再压缩一轮 runtime」只有在 dogfood 证明某概念阻碍日用时才做。
...
3. **抬红线需说明**：若确实需要抬基线，PR 描述必须写明：替换掉的旧概念 + 新基线值，并跑 `make architecture-record` 更新 [`check_concept_growth.py`](../../backend/scripts/check_concept_growth.py) 的 baseline。**禁止静默抬红线。**
