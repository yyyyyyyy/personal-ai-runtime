...
[`backend/app/core/agents/brain.py`](../../backend/app/core/agents/brain.py) 是无状态推理引擎，每个请求一个实例。核心方法 `Brain.chat_stream(conversation, user_message, system_prompt, execution_id, correlation_id)`（[`brain.py:40-271`](../../backend/app/core/agents/brain.py)）是异步生成器，产出 SSE 事件。
...
Failover 执行在 `BrainCompletionMixin._create_llm_stream`（[`brain_completion.py:123-164`](../../backend/app/core/agents/brain_completion.py)）：按 `[primary, *fallbacks]` 顺序尝试，每个失败记录 `LLMCallRecord(success=False)` 遥测；全部失败抛聚合 `RuntimeError`。

### 非流式续接

`BrainCompletionMixin.continue_after_tool_result(conversation, depth)`（[`brain_completion.py:34-121`](../../backend/app/core/agents/brain_completion.py)）在审批解决后做一次性补全，递归深度上限 3。
...
[`backend/app/core/agents/memory_extractor.py`](../../backend/app/core/agents/memory_extractor.py) 是 fire-and-forget 的自动事实抽取。`MemoryExtractor`（[`memory_extractor.py:22`](../../backend/app/core/agents/memory_extractor.py)）持有 `_pending_tasks` 强引用保护 CPython GC。
...
Kernel 拥有 Chroma 索引。`emit_event` 对 `MEMORY_INDEX_EVENT_TYPES` **预计算 embedding_id**，使投影器在同一 SQL 事务写入（[`kernel.py:138-160`](../../backend/app/core/runtime/kernel/kernel.py)）。失败进入 `_pending_memory_index_repairs`（上限 1000），由 [`scripts/verify_vector_consistency.py`](../../backend/scripts/verify_vector_consistency.py) 对账。
...
`_maintenance`（[`runtime_loop.py:177-199`](../../backend/app/core/runtime/runtime_loop.py)）：
1. `agent_registry.cleanup_stale`
2. `kernel.expire_stale_approvals`
3. `_smart_notification_check` — 对停滞 ≥3 天的目标发通知（[`runtime_loop.py:201-239`](../../backend/app/core/runtime/runtime_loop.py)）
4. `_process_background_tasks` — 拉一个 pending background task，emit 状态变更 + `submit_command("BackgroundTaskRequested")`（[`runtime_loop.py:241-270`](../../backend/app/core/runtime/runtime_loop.py)）

cron 表达式解析 `_next_cron_fire(cron_expr, from_ts)`（[`runtime_loop.py:127-173`](../../backend/app/core/runtime/runtime_loop.py)）支持 `minute=*/N`、`hour`/`minute`、`day`、`day_of_week`（名称或数字）。

> APScheduler 在 [`requirements.txt`](../../backend/requirements.txt) 但 core 已迁移到 RuntimeLoop（[`cron_registry.py:1-5`](../../backend/app/core/runtime/cron_registry.py) 注释声明 v0.3.0 起）。

## Cron 注册

[`backend/app/core/runtime/cron_registry.py`](../../backend/app/core/runtime/cron_registry.py) 定义 `SCHEDULES`（[`cron_registry.py:16-25`](../../backend/app/core/runtime/cron_registry.py)）：
...
`init_scheduler()` 还订阅 `TaskCompleted` / `TaskStatusChanged`，自动启动依赖任务（`_on_task_completed`，[`cron_registry.py:42-59`](../../backend/app/core/runtime/cron_registry.py)）。
