      ...
      <div className="p-6 border-b border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1">证明：换模型不会丢掉你的记忆</p>
          </div>
        </div>
      </div>
        ...
        {step === "intro" && (
          <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-6 space-y-4">
            ...
            <p className="text-sm text-gray-300">
              跨模型连续性是 Personal AI Runtime 的核心承诺。你的记忆存储在本地 Event Log
              中，完全独立于任何 LLM 厂商。
            </p>
            {demoData && (
            ...
            {memories.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-500 text-sm">
                  暂无记忆。先与 AI 对话积累记忆，再回来验证。
                </p>
                <p className="text-gray-600 text-xs mt-1">
                  Demo 会展示：切换模型后这些记忆不会丢失。
                </p>
              </div>
            ) : (
          ...
          <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-6 space-y-4">
            ...
            <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4">
              <p className="text-sm text-amber-300">
                去「设置」页面，将默认模型从{" "}
                <code className="px-1 bg-amber-500/20 rounded">
                  {demoData?.model ?? "当前模型"}
                </code>{" "}
                切换为另一个模型（如 Ollama 本地模型），保存后回来点「验证」。
              </p>
            </div>
            <div className="flex gap-3">
              <a
