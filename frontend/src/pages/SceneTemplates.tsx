...
export default function SceneTemplatesPage() {
  ...
  useEffect(() => {
    request<{ templates: Template[] }>(`${API_BASE}/workflows/templates`)
  ...
  const instantiate = async (t: Template) => {
    setCreating(t.id);
    try {
      const r = await request<{ id: string }>(`${API_BASE}/workflows/from-template/${t.id}`, {
        method: "POST",
      });
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {templates
                ...
                .map((t) => (
                  <div
                    key={t.id}
                    className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-5 hover:border-emerald-600/40 transition-colors"
                  >
